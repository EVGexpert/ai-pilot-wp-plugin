# AI Pilot Remote Site API - Complete Endpoint Reference

**Version:** 6.5.0  
**Last Updated:** March 6, 2026  
**Base URL:** `{site_url}/wp-json/aipilot/v1/`  
**Authentication Header:** `X-AIPILOT-RemoteSite-Token: <your-token>`
...
All API requests require authentication via the `X-AIPILOT-RemoteSite-Token` header:

```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-api-token" \
     https://yoursite.com/wp-json/aipilot/v1/site-info
```

**Token Configuration:**
- Generate tokens in WordPress Admin → Settings → AI Pilot Remote API
...
The legacy `X-AI Pilot-Token` header is still supported for v6.5.0 but will be removed in v7.0.0.
...
**Example:**
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     https://yoursite.com/wp-json/aipilot/v1/site-info
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     https://yoursite.com/wp-json/aipilot/v1/diagnostics/health
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     https://yoursite.com/wp-json/aipilot/v1/diagnostics/server
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     https://yoursite.com/wp-json/aipilot/v1/plugins
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     "https://yoursite.com/wp-json/aipilot/v1/posts?status=publish&per_page=10"
```
...
```bash
curl -X POST \
  -H "X-AIPILOT-RemoteSite-Token: your-token" \
  -H "Content-Type: application/json" \
  -d '{"title":"New Post","content":"Content","status":"draft"}' \
  https://yoursite.com/wp-json/aipilot/v1/posts
```
...
curl -X PUT \
  ...
  https://yoursite.com/wp-json/aipilot/v1/posts/123
```
...
curl -X DELETE \
  -H "X-AIPILOT-RemoteSite-Token: your-token" \
  "https://yoursite.com/wp-json/aipilot/v1/posts/123?force=true"
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     "https://yoursite.com/wp-json/aipilot/v1/media?mime_type=image&per_page=10"
```
...
curl -X POST \
  ...
  -F "title=My Image" \
  -F "alt=Alt text" \
  https://yoursite.com/wp-json/aipilot/v1/media
```

**Generating CSRF Token:**
```php
// In WordPress
$csrf_token = wp_create_nonce('aipilot_media_upload');
```
...
curl -X PUT \
  ...
  https://yoursite.com/wp-json/aipilot/v1/media/789
```
...
curl -X DELETE \
  -H "X-AIPILOT-RemoteSite-Token: your-token" \
  "https://yoursite.com/wp-json/aipilot/v1/media/789?force=true"
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     "https://yoursite.com/wp-json/aipilot/v1/crm/subscribers?list_id=1&status=subscribed"
```
...
curl -X POST \
  ...
  https://yoursite.com/wp-json/aipilot/v1/crm/subscribers
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     https://yoursite.com/wp-json/aipilot/v1/crm/subscribers/151
```
...
```bash
curl -H "X-AIPILOT-RemoteSite-Token: your-token" \
     "https://yoursite.com/wp-json/aipilot/v1/support/tickets?status=open"
```
...
curl -X POST \
  -H "X-AIPILOT-RemoteSite-Token: your-token" \
  https://yoursite.com/wp-json/aipilot/v1/self-update
```
...
{
  "status": "updated",
  "message": "AI Pilot Remote API updated successfully",
  "version": "6.5.0"
}
```
...
```json
{
  "url": "https://github.com/aipilot/ai-pilot-plugin/releases/download/v1.0.0/ai-pilot-plugin.zip"
}
```
