# AI Pilot Remote Site API - Permissions Audit Report
...
|---|----------|--------|---------------------|-------|
| 1 | `/diagnostics/health` | GET | `aipilot_api_verify_token` | Token-only check |
| 2 | `/diagnostics/server` | GET | `aipilot_api_verify_token` | Token-only check |

**Notes:**
- Diagnostics module uses basic token verification (`aipilot_api_verify_token`) instead of granular capabilities
...
**Notes:**
- Auth module methods are deprecated in favor of direct `aipilot_verify_token_and_can()` calls
...
### ✅ Token Verification
- [x] All callbacks use `aipilot_verify_token_and_can()`
- [x] Diagnostics module uses `aipilot_api_verify_token()` (appropriate)
...
The AI Pilot Remote Site API v6.5.0 permission system is **production-ready** and meets security best practices for WordPress REST API authentication and authorization.
