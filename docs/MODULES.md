...
This document explains the module architecture of the AI Pilot Remote Site API, including how modules are loaded, activated, and how to create new modules.
...
The AI Pilot Remote Site API uses a modular architecture to integrate with various WordPress plugins and provide REST API endpoints. Each module is responsible for a specific integration (FluentCRM, FluentSupport, Media, etc.).
...
```
┌─────────────────────────────────────────────────────────┐
│              AI Pilot Remote Site API Plugin                  │
├─────────────────────────────────────────────────────────┤
│  Main Plugin File                                        │
...
When WordPress loads the AI Pilot Remote Site API plugin:
...
public static function register_routes() {
    register_rest_route('aipilot/v1', '/crm/subscribers', [
        'methods' => 'GET',
...
<?php
/**
 * AI Pilot Remote Site API - {Module Name} Module
 * 
...
class JRB_{Module_Name}_Module {
    ...
    public static function register_routes() {
        register_rest_route('aipilot/v1', '/{module}/{resource}', [
            'methods' => 'GET',
    ...
    private static function log($message) {
        error_log('[AI Pilot {Module} Module] ' . $message);
    }
}
...
<?php
/**
 * AI Pilot Remote Site API - Example Module
 * 
...
class JRB_Example_Module {
    ...
    public static function register_routes() {
        register_rest_route('aipilot/v1', '/example/items', [
            'methods' => 'GET',
    ...
    private static function log($message) {
        error_log('[AI Pilot Example Module] ' . $message);
    }
}
...
3. **Test endpoints:**
   ...
   curl -H "X-JRBRemoteSite-Token: your-token" \
        https://yoursite.com/wp-json/aipilot/v1/example/items
...
3. **Minimal Routes:** Register only necessary endpoints
   ...
   register_rest_route('aipilot/v1', '/crm/subscribers', [...]);
   register_rest_route('aipilot/v1', '/crm/subscribers/(?P<id>\d+)', [...]);
   ```
...
4. Check error log for module loading messages:
   ```bash
   tail -f wp-content/debug.log | grep "AI Pilot.*Module"
   ```
