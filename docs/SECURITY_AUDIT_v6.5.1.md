# AI Pilot Remote Site API v6.5.1 - Security Audit Report
...
**Previous State:**
- Two separate token stores: `aipilot_api_token_hash` and `aipilot_api_token_hash`
- Two different headers: `X-JRBRemoteSite-Token` and `X-AI Pilot-Token`
...
**Verification:**
```bash
grep -n "aipilot_api_token_hash\|aipilot_api_token_hash" src/Auth/Guard.php
# Result: Primary uses aipilot_api_token_hash, legacy auto-migrates
```
...
**Previous State:**
- Three namespaces active: `aipilot/v1`, `aipilot/v1`, `aipilot/v1`
...
const VERSION = '6.5.1';
const API_NAMESPACE = 'aipilot/v1';  // Single source of truth
const LEGACY_NAMESPACES = ['aipilot/v1', 'aipilot/v1'];  // Documented deprecation
```

**Verification:**
```bash
grep "API_NAMESPACE" src/Core/Plugin.php src/Handlers/*.php
# Result: All handlers now use consistent aipilot/v1 namespace
```
