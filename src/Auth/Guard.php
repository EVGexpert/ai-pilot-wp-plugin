<?php
namespace AIPILOT\RemoteApi\Auth;
 ...
 *
 * Primary: X-AIPILOT-RemoteSite-Token header + aipilot_api_token_hash
 * Legacy: X-AIPILOT-Token header + aipilot_api_token_hash (migrated on first use)
 *
 * @since 6.5.1
...
class Guard {
    ...
    public static function verify_token() {
        ...
        $cached_result = get_transient('aipilot_token_verified');
        ...
        if (empty($header)) {
            $header = isset($_SERVER['HTTP_X_AIPILOT_TOKEN'])
                ? sanitize_text_field(wp_unslash($_SERVER['HTTP_X_AIPILOT_TOKEN']))
                : '';
        }

        if (empty($header)) {
            self::$token_verified_cache = new \WP_Error('missing_header', 'Missing X-AIPILOT-RemoteSite-Token header (or legacy X-AIPILOT-Token)', ['status' => 401]);
            return self::$token_verified_cache;
        }

        // Check PRIMARY token storage (aipilot_api_token_hash)
        ...
        if (!empty($token_hash)) {
            if (hash_equals($token_hash, wp_hash($header))) {
                ...
                set_transient('aipilot_token_verified', 'valid', self::CACHE_TTL);
                self::$token_verified_cache = true;
                return true;
            }
        }
        ...
        $legacy_hash = get_option('aipilot_api_token_hash');
        if (!empty($legacy_hash)) {
            if (hash_equals($legacy_hash, wp_hash($header))) {
                // Migrate to primary storage
                update_option('aipilot_api_token_hash', $legacy_hash);
                delete_option('aipilot_api_token_hash');
                delete_option('aipilot_api_token'); // Remove any legacy plaintext

                // Log migration event
                error_log('[AI Pilot] Auth token migrated from aipilot_api_token_hash to aipilot_api_token_hash');
        ...
        $legacy_token = get_option('aipilot_api_token');
        if (!empty($legacy_token)) {
            if (hash_equals($legacy_token, $header)) {
                // Migrate plaintext to hashed
                update_option('aipilot_api_token_hash', wp_hash($legacy_token));
                delete_option('aipilot_api_token');

                error_log('[AI Pilot] Plaintext token migrated to hashed storage');
    ...
    public static function can($capability) {
        ...
        if (self::$capabilities_cache === null) {
            ...
            $cached = get_transient('aipilot_capabilities_cache');
            if ($cached !== false) {
                self::$capabilities_cache = $cached;
            } else {
                // Load from database
                $caps = get_option('aipilot_api_capabilities', []);
                ...
                set_transient('aipilot_capabilities_cache', $caps, self::CACHE_TTL);
            }
        }
    ...
    public static function clear_auth_cache() {
        ...
        delete_transient('aipilot_token_verified');
        delete_transient('aipilot_capabilities_cache');
    }
    ...
    public static function migrate_legacy_auth() {
        $old_hash = get_option('aipilot_api_token_hash');
        $new_hash = get_option('aipilot_api_token_hash');

        // Migrate token hash if new doesn't exist
        if ($old_hash && !$new_hash) {
            update_option('aipilot_api_token_hash', $old_hash);
            delete_option('aipilot_api_token_hash');
            error_log('[AI Pilot] Token hash migrated during update');
        }

        // Migrate capabilities
        $old_caps = get_option('aipilot_api_capabilities');
        $new_caps = get_option('aipilot_api_capabilities');

        if ($old_caps && !$new_caps) {
            update_option('aipilot_api_capabilities', $old_caps);
            // Keep legacy caps for backward compat during transition
            error_log('[AI Pilot] Capabilities migrated during update');
        }
