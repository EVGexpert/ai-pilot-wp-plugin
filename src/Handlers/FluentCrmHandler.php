...
use AIPILOT\RemoteApi\Auth\Guard;
 ...
 *
 * @since 6.5.1
 * @package AIPILOT\RemoteApi\Handlers
 */
class FluentCrmHandler {
    ...
    private static function get_cache_key($query_type, $params) {
        return 'aipilot_crm_' . $query_type . '_' . md5(serialize($params));
    }
    ...
    private static function get_cached_query($key) {
        ...
        $cached = wp_cache_get($key, 'aipilot_crm');
        if ($cached !== false) {
    ...
    private static function set_cached_query($key, $data) {
        ...
        wp_cache_set($key, $data, 'aipilot_crm', self::QUERY_CACHE_TTL);
    }
    ...
    public static function list_subscribers($request) {
        ...
        if (!self::validate_table_name($table)) {
            error_log('[AI Pilot] Invalid table name attempted: ' . $table);
            return new \WP_Error(
                'invalid_table',
                'Invalid table name',
                ['status' => 400]
        ...
        try {
            ...
            $response_data = [
                ...
                'meta' => [
                    ...
                    'namespace' => \AIPILOT\RemoteApi\Core\Plugin::API_NAMESPACE,
                    'cached' => false,
                ],
            ];
            ...
            error_log('[AI Pilot] Database error in list_subscribers: ' . $e->getMessage());
