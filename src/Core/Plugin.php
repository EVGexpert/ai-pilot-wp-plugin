<?php
namespace AIPILOT\RemoteApi\Core;
 ...
 * Core Plugin Class
 *
 * Central configuration and initialization for AI Pilot Site API
 * Optimized v6.5.1 with caching and performance improvements
 *
 * @since 6.5.1
 * @package AIPILOT\RemoteApi\Core
 */
class Plugin {
    /**
    ...
    public static function init() {
        ...
        if (defined('AIPILOT_CORE_INITIALIZED')) {
            return;
        }
        define('AIPILOT_CORE_INITIALIZED', true);
        ...
        // Register admin menu
        if (is_admin()) {
            \AIPILOT\RemoteApi\Handlers\AdminHandler::init();
        }
        ...
        add_action('update_option_aipilot_api_capabilities', [self::class, 'clear_capabilities_cache'], 10, 3);
        add_action('deleted_option_aipilot_api_capabilities', [self::class, 'clear_capabilities_cache']);
    }
    ...
    public static function migrate_legacy_auth() {
        ...
        if (get_transient('aipilot_auth_migration_done')) {
            return;
        }

        // Delegate to Guard class for migration logic
        if (class_exists('\\AIPILOT\\RemoteApi\\Auth\\Guard')) {
            \AIPILOT\RemoteApi\Auth\Guard::migrate_legacy_auth();
        }

        // Set transient to prevent repeated migration checks
        set_transient('aipilot_auth_migration_done', true, HOUR_IN_SECONDS);
    }
    ...
    public static function register_routes() {
        ...
        \AIPILOT\RemoteApi\Handlers\SystemHandler::register_routes();
        \AIPILOT\RemoteApi\Handlers\AdminHandler::register_routes();

        // 🚀 OPTIMIZATION: Conditionally load integration handlers
        if (class_exists('\\AIPILOT\\RemoteApi\\Handlers\\FluentCrmHandler')) {
            \AIPILOT\RemoteApi\Handlers\FluentCrmHandler::register_routes();
        }

        if (class_exists('\\AIPILOT\\RemoteApi\\Handlers\\FluentSupportHandler')) {
            \AIPILOT\RemoteApi\Handlers\FluentSupportHandler::register_routes();
        }
    }
    ...
    public static function add_api_headers($response, $handler, $request) {
        ...
        $response->header('X-AIPILOT-API-Version', self::VERSION);
        $response->header('X-AIPILOT-API-Namespace', self::API_NAMESPACE);
    ...
    public static function get_capabilities() {
        ...
        set_transient('aipilot_capabilities_cache', $caps, 5 * MINUTE_IN_SECONDS);
    ...
    public static function get_plugin_info() {
        ...
        self::$plugin_info_cache = get_plugin_data(__DIR__ . '/../../ai-pilot.php');
        return self::$plugin_info_cache;
    }
