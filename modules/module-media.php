 ...
 */
function aipilot_generate_media_csrf_token() {
    return wp_create_nonce('aipilot_media_upload');
}

/**
 * Security: Verify CSRF token for upload requests
 */
function aipilot_verify_media_csrf($request) {
    $token = $request->get_header('X-CSRF-Token') ?: $request->get_param('csrf_token');
    if (empty($token) || !wp_verify_nonce($token, 'aipilot_media_upload')) {
        return false;
    }
    return true;
}
...
add_action('rest_api_init', function() {
    // List media
    aipilot_register_route('/media', [
        'methods' => 'GET',
        'callback' => 'aipilot_media_list',
        'permission_callback' => function() { return aipilot_verify_token_and_can('media_read'); },
        'args' => [
            'page' => [
                'type' => 'integer',
                'default' => 1,
    ...
    aipilot_register_route('/media', [
        ...
        'permission_callback' => function() { 
            ...
            return aipilot_verify_token_and_can('media_upload'); 
        },
    ]);
    
    // Get single media
    aipilot_register_route('/media/(?P<id>\d+)', [
        'methods' => 'GET',
        'callback' => 'aipilot_media_get',
        'permission_callback' => function() { return aipilot_verify_token_and_can('media_read'); },
        'args' => [
            'id' => [
                'type' => 'integer',
    ...
    aipilot_register_route('/media/(?P<id>\d+)', [
        'methods' => 'PUT',
        'callback' => 'aipilot_media_update',
        'permission_callback' => function() { return aipilot_verify_token_and_can('media_edit'); },
        'args' => [
            'id' => [
                'type' => 'integer',
                'sanitize_callback' => 'absint',
    ...
    aipilot_register_route('/media/(?P<id>\d+)', [
        'methods' => 'DELETE',
        'callback' => 'aipilot_media_delete',
        'permission_callback' => function() { return aipilot_verify_token_and_can('media_delete'); },
        'args' => [
            'id' => [
                'type' => 'integer',
                'sanitize_callback' => 'absint',
 ...
 */
function aipilot_get_allowed_mime_types() {
    ...
    return apply_filters('aipilot_allowed_mime_types', $allowed);
}
 ...
 */
function aipilot_get_max_upload_size() {
    // Default 10MB, filterable
    $max_size = apply_filters('aipilot_max_upload_size', 10 * 1024 * 1024);
 ...
 */
function aipilot_sanitize_filename($filename) {
 ...
 */
function aipilot_validate_svg($file_path) {
    // SVGs can contain malicious scripts, validate carefully
    // CRITICAL: Limit file read to prevent DoS via huge files
    $max_size = apply_filters('aipilot_svg_validation_max_size', 256 * 1024); // 256KB max for SVG scanning
    $file_size = filesize($file_path);
    
    if ($file_size > $max_size) {
        return new WP_Error(
            'svg_too_large',
            sprintf(__('SVG file exceeds maximum size of %s for validation.', 'ai-pilot'), size_format($max_size))
        );
    }
    ...
    if ($content === false) {
        return new WP_Error(
            'svg_read_failed',
            __('Failed to read SVG file for security validation.', 'ai-pilot')
        );
    }
    ...
    foreach ($dangerous as $pattern) {
        if (preg_match($pattern, $content)) {
            return new WP_Error(
                'unsafe_svg',
                __('This SVG file contains potentially unsafe content that could lead to XSS attacks.', 'ai-pilot')
            );
        }
    }
    ...
    if ($xml_elements > 5000) {
        ...
        error_log(sprintf('AI Pilot Media: Large SVG detected with %d elements', $xml_elements));
    }
 ...
 */
function aipilot_media_list($request) {
    $args = [
    ...
    foreach ($query->posts as $post) {
        $items[] = aipilot_format_media_item($post);
    }
 ...
 */
function aipilot_format_media_item($post) {
 ...
 */
function aipilot_media_upload($request) {
    ...
    if (empty($files)) {
        ...
        if ($disposition && $request->get_body()) {
            ...
            if (preg_match('/filename="?([^";]+)"?/i', $disposition, $matches)) {
                ...
                $max_upload = aipilot_get_max_upload_size();
                ...
                if ($content_length) {
                    ...
                    if ($content_length > $max_upload) {
                        return new WP_Error(
                            'content_too_large',
                            __('Request body exceeds maximum allowed size.', 'ai-pilot'),
                            ['status' => 413]
                        );
                    }
                }
    ...
    if (empty($file) || $file['error'] !== 0) {
        return new WP_Error(
            'upload_error',
            __('No file uploaded or upload error.', 'ai-pilot'),
            ['status' => 400]
        );
    }
    ...
    $max_size = aipilot_get_max_upload_size();
    
    if ($file_size === false || $file_size > $max_size) {
        return new WP_Error(
            'file_too_large',
            sprintf(
                __('File size exceeds maximum of %s.', 'ai-pilot'),
    ...
    $allowed_types = aipilot_get_allowed_mime_types();
    ...
    if ($file_type['type'] !== $actual_mime && !empty($actual_mime)) {
        ...
        return new WP_Error(
            'mime_mismatch',
            __('File MIME type verification failed.', 'ai-pilot'),
            ['status' => 400]
        );
    }
    ...
    if (!in_array($file_type['type'], $allowed_types, true)) {
        return new WP_Error(
            'invalid_file_type',
            sprintf(
                __('File type "%s" is not allowed. Allowed types: %s', 'ai-pilot'),
                $file_type['type'],
    ...
    if ($file_type['type'] === 'image/svg+xml') {
        $svg_check = aipilot_validate_svg($file['tmp_name']);
    ...
    if ($file_content && preg_match('/<\?php|<\?=|<\s*script/i', $file_content)) {
        return new WP_Error(
            'suspicious_content',
            __('File contains suspicious code that could be executed on the server.', 'ai-pilot'),
            ['status' => 400]
        );
    }
    
    // Sanitize filename (path traversal prevention)
    $filename = aipilot_sanitize_filename($file['name']);
    ...
    if (strpos($relative_path, '..') !== false || strpos($relative_path, '/') === 0) {
        ...
        return new WP_Error(
            'upload_security',
            __('Upload directory traversal attempt detected.', 'ai-pilot'),
            ['status' => 403]
        );
    }
    ...
    return new WP_REST_Response(
        aipilot_format_media_item($post),
        201
    );
}
 ...
 * @return WP_REST_Response|WP_Error
 */
function aipilot_media_get($request) {
    ...
    if (!$post || $post->post_type !== 'attachment') {
        return new WP_Error(
            'not_found',
            __('Media item not found.', 'ai-pilot'),
            ['status' => 404]
        );
    }
    
    return new WP_REST_Response(aipilot_format_media_item($post));
}
 ...
 * @return WP_REST_Response|WP_Error
 */
function aipilot_media_update($request) {
    $id = $request->get_param('id');
    $post = get_post($id);
 ...
 * @return WP_REST_Response|WP_Error
 */
function aipilot_media_delete($request) {
    $id = $request->get_param('id');
    ...
    if (!$result) {
        return new WP_Error(
            'delete_failed',
            __('Failed to delete media item.', 'ai-pilot'),
            ['status' => 500]
        );
    }
    ...
    return new WP_REST_Response([
        'deleted' => true,
        'id' => $id,
        'previous' => aipilot_format_media_item($post),
    ]);
}